...
function openDrawer(vendorCode, division){
  ...
  // 대시보드에서 특정 연도를 선택 중이면 그 연도로 시작, '전체'면 최신 연도로 시작
  const initialYear = (state.yearFilter !== 'ALL' && rows.some(r => r.year === state.yearFilter))
    ? state.yearFilter : latest.year;

  renderDrawerBody(vendorCode, division, initialYear);

  document.getElementById('overlay').classList.add('open');
  document.getElementById('drawer').classList.add('open');
}

function renderDrawerBody(vendorCode, division, focusYear){
  const rows = findYoY(vendorCode, division);
  if (!rows.length) return;
  const focusRow = rows.find(r => r.year === focusYear) || rows[rows.length-1];
  ...
  const stampHtml = rows.map((r, i) => {
    ...
    const stamp = `
      <div class="stamp${isFocus ? ' stamp-focus' : ''}" data-year="${r.year}" style="border-color:${GRADE_COLORS[gradeLetter(r.finalGrade)]}; color:${GRADE_COLORS[gradeLetter(r.finalGrade)]}">
  ...
  const trendHtml = rows.length >= 2 ? `
    <div class="trend-panel">
      <h4>평가점수 추이 (연도별) · 카드를 눌러 아래 정보 전환</h4>
  ...
  // 연도 카드를 누르면 그 연도로 하단 정보 전환
  document.querySelectorAll('#drawer-body .stamp[data-year]').forEach(el => {
    el.style.cursor = 'pointer';
    el.addEventListener('click', () => {
      renderDrawerBody(vendorCode, division, Number(el.dataset.year));
    });
  });
      ...
      chartDrawerTrend = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            pointBorderColor: rows.map(r => r.year === focusRow.year ? '#1c3c58' : '#fff'),
            pointBorderWidth: rows.map(r => r.year === focusRow.year ? 3 : 2),
            pointRadius: rows.map(r => r.year === focusRow.year ? 7 : 6),
            pointHoverRadius: 8,
            borderWidth: 2,
            tension: 0.25,
            fill: false
        ...
        options: {
          ...
          onClick: (evt, elements) => {
            if (elements.length){
              renderDrawerBody(vendorCode, division, rows[elements[0].index].year);
            }
          },
          plugins: {
